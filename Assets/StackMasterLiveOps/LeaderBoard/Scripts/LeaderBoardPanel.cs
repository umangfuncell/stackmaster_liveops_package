...
namespace FunCell.GridMerge.UI
{
    public class LeaderboardPanel : Panel
    {
        ...
        [SerializeField] Sprite[] _countryIcons;
        ...
        private void SetupDummyNames()
        {
            _leaderboardNames = new string[]
            {
                "Smith","Anderson","Clark","Wright","Mitchell","Johnson","Thomas","Rodriguez","Lopez",
                "Perez","Williams","Jackson","Lewis","Hill","Roberts","Jones","White","Lee","Scott",
                "Turner","Brown","Harris","Walker","Green","Phillips","Davis","Martin","Hall","Adams"
            };
        }
        private string GetRandomName()
        {
            return _leaderboardNames[Random.Range(0, _leaderboardNames.Length)];
        }

        private void SpawnPlayerBelow()
        {
        ...
        //        {
        //            //rank = (int)ExtensionMethods.Map(score, _minScore, _maxScore, category.MaxRank, category.MinRank);
        //            Debug.Log( "Lerp Rank: "+Mathf.Lerp(category.MaxRank, category.MinRank,(float) score /(float) category.MaxScore));
        //            rank =(int)Mathf.Lerp(category.MaxRank, category.MinRank, (float)score- category.MinScore / (float)category.MaxScore- category.MinScore);
        //            break;
        //        }
        //    }
        //    return rank;
        ...
        {
            string tmpSound = PlayerPrefs.GetString("Sound");
            string tmpMusic = PlayerPrefs.GetString("Music");
            SuccessCallbacK(true);
            void SuccessCallbacK(bool isSucess)
            {
                // Debug.Log("Jump rank success callback");
                if (LeanTween.isTweening(leaderboardTweenId))
                {
                    LeanTween.cancel(leaderboardTweenId, true);
                }
                ...
                {
                    if (i != 0 && i != 3)
                        _contentObj.GetChild(i).SetParent(tempObj.transform);
                }

                Destroy(tempObj);

                float inc = 535000 / 5000;
        ...
        {
            OpsGameManager.Instance.ChangeStateTo(GameState.Over);
        }
    }
}
[System.Serializable]
